	...
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				usr.save()
				if username.isdigit():
					SalerDetail(user=usr,mobile=username).save()    #	SalerDetail(user=usr,mobile=username,gst_Number=gst,shop_Name=gst).save()
				else:
					usr.email = username
					usr.save()
					SalerDetail(user=usr).save()      #SalerDetail(user=usr,gst_Number=gst,shop_Name=gst).save()
				messages.success(request, f'Account is Created for {username}')
				return redirect('login')
		else:
			form = SalerRegisterForm()
	return render(request, 'saler/seller_signup.html', {'form':form, 'title':'ISKON Member'})
	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			...
			for col in range(0, dataframe1.max_row):
				...
				specs=ROW[18]
				URL=ROW[20]
				image1 = 'products/images/idli.jpg'
				if Product.objects.all():
					prod_id2 = 'pr'+hex(Product.objects.all().last().product_id+1)
				else:
					prod_id2 = 'pr'+hex(0)
				Product(product_id2=prod_id2, shop=request.user, product_name=prod_name,ingredients=ingredients,TotalTime=TotalTime,
						...
						diet=diet,TotalCal=TotalCal,specs=specs,URL=URL,allergies=allergies,
						category=subcat, subcategory=cat,image1=image1).save()
				# p = Product.objects.filter(product_id2=prod_id2)[0]
				#
				# p.save()
				ROW=[]
